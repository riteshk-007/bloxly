...
import { Calendar, Clock, ArrowLeft, Tag, User, Hash } from 'lucide-react';
...
export default function TagPage() {
    ...
    const fetchPosts = useCallback(async (page = 1) => {
        try {
            setLoading(true);
            const response = await fetch(`/api/public/posts?tag=${slug}&page=${page}&limit=10`);
            if (response.ok) {
                ...
                if (data.posts.length > 0) {
                    const foundTag = data.posts[0].tags?.find(t => t.slug === slug);
                    if (foundTag) {
                        setTag(foundTag);
                    }
                }
                ...
                <div className="mb-8">
                    ...
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-yellow-200">
                        <div className="flex items-center mb-2">
                            <Hash className="w-6 h-6 text-yellow-500 mr-2" />
                ...
                {posts.length > 0 ? (
                            ...
                            <article key={post.id} className="bg-white rounded-xl p-6 shadow-sm border border-yellow-200 hover:shadow-md transition-shadow">
                                <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 mb-3">
                                    ...
                                    {post.category && (
                                        <Link
                                            ...
                                            className="flex items-center px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs hover:bg-yellow-200 transition-colors"
                                        >
                                ...
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {post.tags?.map((postTag) => (
                                        <Link
                                            key={postTag.id}
                                            href={`/blog/tag/${postTag.slug}`}
                                            className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium transition-colors ${postTag.slug === slug
                    ...
                    <div className="text-center py-12">
                        <div className="bg-white rounded-xl p-8 shadow-sm border border-yellow-200">
                            ...
                            <p className="text-gray-600 mb-4">There are no published posts with this tag yet.</p>
                            <Link
                                href="/blog"
